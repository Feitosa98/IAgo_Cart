...
CNS_CODIGO = "004879"  # <-- SUBSTITUA PELO CNS REAL DA SERVENTIA
...
def abrir_arquivo_no_sistema(file_path: str):
    """Abre o arquivo no visualizador padrão, corrigindo caminho de rede/UNC."""
    try:
        # Normalizar barras
...
def ocr_tiff_to_text(tiff_path: str) -> str:
    ...
    # sem lang="por" para evitar erro se não tiver pacote de idioma
...
def row_to_indicador_item(row, tipoenvio: int) -> dict:
    """
    Converte uma linha do banco para o item JSON no formato do manual ONR.
    """
    base = row_to_json(row)

    return {
        "TIPOENVIO": tipoenvio,  # 0=ATUAL, 1=ATUALIZAÇÃO
        "NUMERO_REGISTRO": base["NUMERO_REGISTRO"] or "",
        "VARIOS_ENDERECOS": base["VARIOS_ENDERECOS"] or "",
        "REGISTRO_TIPO": base["REGISTRO_TIPO"] or 1,
        "TIPO_DE_IMOVEL": base["TIPO_DE_IMOVEL"] or 0,
        "LOCALIZACAO": base["LOCALIZACAO"] or 0,
        "TIPO_LOGRADOURO": base["TIPO_LOGRADOURO"] or 0,
        "NOME_LOGRADOURO": base["NOME_LOGRADOURO"] or "",
        "NUMERO_LOGRADOURO": base["NUMERO_LOGRADOURO"] or "",
        "UF": base["UF"] or 0,
        "CIDADE": base["CIDADE"] or 0,
        "BAIRRO": base["BAIRRO"] or "",
        "CEP": (base["CEP"] or "").replace("-", ""),
        "COMPLEMENTO": base["COMPLEMENTO"] or "",
        "QUADRA": base["QUADRA"] or "",
        "CONJUNTO": base["CONJUNTO"] or "",
        "SETOR": base["SETOR"] or "",
        "LOTE": base["LOTE"] or "",
        "LOTEAMENTO": base["LOTEAMENTO"] or "",
        "CONTRIBUINTE": [str(c) for c in base["CONTRIBUINTE"]],
        "RURAL": base["RURAL"],
...
def export_to_json(db_path: str, output_path: str, since: str | None = None, tipoenvio: int = 0):
    """
    Gera JSON no formato do manual do Indicador Real:
    {
      "INDICADOR_REAL": {
        "CNS": "xxxxxx",
        "REAL": [ {...}, {...} ]
...
class ImoveisApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.progress = ttk.Progressbar(bottom_frame, orient="horizontal", mode="determinate", length=400)
        self.progress.pack(side="left", padx=5)

        self.progress_label = ttk.Label(bottom_frame, text="Pronto.")
    ...
    def importar_tiff(self):
        ...
        self.progress_label.config(text=f"Importando 0 de {total} matrículas...")
        ...
        for i, file_path in enumerate(file_paths, start=1):
            ...
            self.progress_label.config(text=f"Importando {i} de {total} matrículas...")
            self.update_idletasks()

        self.load_registros()

        resumo = f"{sucesso} de {total} matrícula(s) importadas com sucesso."
        if erros:
            resumo += "\n\nOcorreram erros em:\n" + "\n".join(erros)

        self.progress_label.config(text=f"Importação concluída: {sucesso} de {total}.")
    ...
    def exportar_json_completo(self):
        ...
        if not filename:
            return
        try:
            # TIPOENVIO 0 = ATUAL (carga completa)
            export_to_json(self.db_path, filename, since=None, tipoenvio=0)
    ...
    def exportar_json_alteracoes(self):
        ...
        if not filename:
            return
        try:
            # TIPOENVIO 1 = ATUALIZAÇÃO (últimas alterações)
            export_to_json(self.db_path, filename, since=since, tipoenvio=1)
            messagebox.showinfo("Sucesso", f"JSON exportado para {filename}")
        except Exception as e:
            messagebox.showerror("Erro ao exportar JSON", str(e))
