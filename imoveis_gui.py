...
# Cidades reconhecidas automaticamente (pode adicionar mais depois)
CIDADE_NOME_TO_IBGE = {
    "MANACAPURU": 1302504,
    # "MANAUS": 1302603,
    # "IRANDUBA": 1301852,
...
def parse_text_to_dict(text: str) -> dict:
    ...
    # 2) Tenta achar cidade pelo nome (MANACAPURU, etc.)
...
def save_dict_to_db(data: dict, tiff_path: str | None = None, db_path: str = DB_DEFAULT) -> int:
    ...
    cur.execute(
        ...
        (
            ...
            data["VARIOS_ENDERECOS"],
            data["REGISTRO_TIPO"],
            data["TIPO_DE_IMOVEL"],
            data["LOCALIZACAO"],
            data["TIPO_LOGRADOURO"],
            data["NOME_LOGRADOURO"],
...
def update_imovel(idreg: int, campos: dict, db_path: str = DB_DEFAULT):
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def row_to_json(row: tuple) -> dict:
    keys = [
        "id", "numero_registro", "varios_enderecos", "registro_tipo", "tipo_de_imovel",
        "localizacao", "tipo_logradouro", "nome_logradouro", "numero_logradouro", "uf",
        "cidade", "bairro", "cep", "complemento", "quadra", "conjunto", "setor",
        "lote", "loteamento", "contribuinte",
        "rural_car", "rural_nirf", "rural_ccir", "rural_numero_incra",
        "rural_sigef", "rural_denominacaorural", "rural_acidentegeografico",
        "condominio_nome", "condominio_bloco", "condominio_conjunto",
        "condominio_torre", "condominio_apto", "condominio_vaga",
    ...
    data["contribuinte"] = json.loads(data["contribuinte"]) if data["contribuinte"] else []

    data["RURAL"] = {
        "CAR": data["rural_car"] or "",
        "NIRF": data["rural_nirf"] or "",
        "CCIR": data["rural_ccir"] or "",
        "NUMERO_INCRA": data["rural_numero_incra"] or "",
        "SIGEF": data["rural_sigef"] or "",
        "DENOMINACAORURAL": data["rural_denominacaorural"] or "",
        "ACIDENTEGEOGRAFICO": data["rural_acidentegeografico"] or "",
    }

    data["CONDOMINIO"] = {
        "NOME_CONDOMINIO": data["condominio_nome"] or "",
        "BLOCO": data["condominio_bloco"] or "",
        "CONJUNTO": data["condominio_conjunto"] or "",
        "TORRE": data["condominio_torre"] or "",
        "APTO": data["condominio_apto"] or "",
        "VAGA": data["condominio_vaga"] or "",
    }

    return data


def row_to_indicador_item(row: tuple, tipoenvio: int) -> dict:
...
def export_to_json(db_path: str, output_path: str, since: str | None = None, tipoenvio: int = 0):
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
class ImoveisApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        def add_combo(row: int, label: str, key: str, options: list[str]):
            ttk.Label(right, text=label + ":").grid(row=row, column=0, sticky="e", padx=5, pady=2)
        ...
        add_entry(row, "Matrícula", "NUMERO_REGISTRO")
        row += 1
        add_combo(row, "Tipo Registro", "REGISTRO_TIPO", make_combo_values(REGISTRO_TIPO_OPCOES))
        row += 1
        add_combo(row, "Localização", "LOCALIZACAO", make_combo_values(LOCALIZACAO_OPCOES))
        row += 1
        add_combo(row, "Tipo Imóvel", "TIPO_DE_IMOVEL", make_combo_values(TIPO_IMOVEL_OPCOES))
        row += 1
        add_combo(row, "Tipo Logradouro", "TIPO_LOGRADOURO", make_combo_values(TIPO_LOGRADOURO_OPCOES))
        row += 1

        add_entry(row, "Nome Logradouro", "NOME_LOGRADOURO")
        row += 1
        add_entry(row, "Número Logradouro", "NUMERO_LOGRADOURO")
        row += 1
        add_combo(row, "UF", "UF", make_combo_values(UF_OPCOES))
        row += 1
        add_entry(row, "Cidade (IBGE)", "CIDADE")
        row += 1

        add_entry(row, "Bairro", "BAIRRO")
        row += 1
        add_entry(row, "CEP", "CEP")
        row += 1
        add_entry(row, "Complemento", "COMPLEMENTO")
        row += 1
        add_entry(row, "Quadra", "QUADRA")
        row += 1
        add_entry(row, "Conjunto", "CONJUNTO")
        row += 1
        add_entry(row, "Setor", "SETOR")
        row += 1
        add_entry(row, "Lote", "LOTE")
        row += 1
        add_entry(row, "Loteamento", "LOTEAMENTO")
        row += 1
        add_entry(row, "Vários Endereços (S/N)", "VARIOS_ENDERECOS")
        row += 1
        add_entry(row, "Contribuinte(s)", "CONTRIBUINTE")
        row += 1
    ...
    def carregar_registro(self):
        ...
        def set_entry(k: str, val):
            w = self.entries.get(k)
            if w:
                w.delete(0, tk.END)
                w.insert(0, "" if val is None else str(val))

        def set_combo(k: str, codigo: int | None, mapping: dict):
    ...
    def salvar_alteracoes(self):
        ...
        try:
            ...
            campos["registro_tipo"] = combo_to_codigo(self.combos["REGISTRO_TIPO"].get()) or 1
            campos["localizacao"] = combo_to_codigo(self.combos["LOCALIZACAO"].get()) or 0
            campos["tipo_de_imovel"] = combo_to_codigo(self.combos["TIPO_DE_IMOVEL"].get()) or 0
            campos["tipo_logradouro"] = combo_to_codigo(self.combos["TIPO_LOGRADOURO"].get()) or 250
            campos["uf"] = combo_to_codigo(self.combos["UF"].get()) or 13
            ...
            messagebox.showerror("Erro", "Campos numéricos inválidos (ex.: Cidade deve ser código IBGE).")
        except Exception as e:
